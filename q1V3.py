...
class Passenger:
    ...
    def __str__(self):
        return f"Name: {self.name}, Passport: {self.passport}, DOB: {self.DOB}, Membership Tier: {self.membership_tier}"
    ...
    def add_passenger_to_flight(self, passport): 
        ...
        if passenger:
            for flight_number, flight in self.flights.items():
                ...
                if occupancy_rate < 0.8:  # Stop adding passengers when the flight is 80% full
        ...
        if choice == '1':
            ...
            if flight:
                ...
                while len(flight.confirmed_passengers) < flight.max_capacity:
                    ...
                    if passenger:
                        print(f"Passenger {passenger.name} with {passenger.membership_tier} assigned to flight {flight_number}")
                    else:
